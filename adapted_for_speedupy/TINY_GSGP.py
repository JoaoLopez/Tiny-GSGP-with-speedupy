...
def randfunct():
    ...
    re = randexpr(DEPTH)
    temp1 = ', '
    rf = eval('lambda ' + temp1.join(vars) + ': ' + re) # create function of n input variables
...
def mutation(p):
    ...
    temp2 = ' and '
    mintermexpr = temp2.join([random.choice([x,'not ' + x]) for x in vars]) # random minterm expression of n variables
    temp3 = ', '
    minterm = eval('lambda ' + temp3.join(vars) + ': ' + mintermexpr) # turn minterm into a function
